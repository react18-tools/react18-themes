...
describe("nextjs-server-theme-switcher", () => {
	afterEach(cleanup);
	...
	test("test default tag", ({ expect }) => {
		...
		expect(screen.getByTestId("nextjs-server-theme-switcher").tagName).toBe("DIV");
	});
	test("test default tag for ServerSideWrapper", ({ expect }) => {
		render(<ServerSideWrapper />);
		expect(screen.getByTestId("nextjs-server-theme-switcher").tagName).toBe("HTML");
	});
	test("test custom tag", ({ expect }) => {
		render(<NextJsSSGThemeSwitcher tag="main" />);
		expect(screen.getByTestId("nextjs-server-theme-switcher").tagName).toBe("MAIN");
	});
	test("forced theme", ({ expect }) => {
		...
		expect(screen.getByTestId("nextjs-server-theme-switcher").getAttribute("data-theme")).toBe("dark1");
	});
	...
	test("forced color scheme dark", ({ expect }) => {
		...
		expect(screen.getByTestId("nextjs-server-theme-switcher").getAttribute("data-theme")).toBe("dark-blue");
		expect(screen.getByTestId("nextjs-server-theme-switcher").getAttribute("data-color-scheme")).toBe("dark");
	});
	test("forced color scheme light", ({ expect }) => {
		...
		expect(screen.getByTestId("nextjs-server-theme-switcher").getAttribute("data-theme")).toBe("light-yellow");
	});
	...
	test("force disable color scheme", ({ expect }) => {
		...
		expect(screen.getByTestId("nextjs-server-theme-switcher").getAttribute("data-theme")).toBe("yellow");
	});
	...
	test("force disable color scheme", ({ expect }) => {
		...
		globalThis.path = "/forced-color-scheme";
		render(<NextJsSSGThemeSwitcher forcedPages={[[/forced-color-scheme$/, { colorScheme: "" }]]} />);
		expect(screen.getByTestId("nextjs-server-theme-switcher").getAttribute("data-color-scheme")).toBe("");
	});
});
